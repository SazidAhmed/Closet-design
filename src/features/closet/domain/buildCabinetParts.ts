...
export function getCabinetDerivedDims(state: ClosetStateV2): CabinetDerivedDims {
  ...
  return { width, height, depth, thickness, innerWidth, innerHeight, innerDepth }
}
...
export function buildParts(state: ClosetStateV2): ClosetPart[] {
  const { width: W, height: H, depth: D, thickness: T, innerWidth: iW, innerHeight: iH, innerDepth: iD } =
  ...
  for (let ti = 0; ti < state.towers.length; ti++) {
    ...
    buildTowerAccessories(parts, tower, towerCenterX, tw, T, H, D, iD, iH, finishId)
...
function buildTowerAccessories(
  ...
  for (const acc of tower.accessories) {
    switch (acc.type) {
      case 'shelf_set':
        buildShelves(parts, tower, acc.count, centerX, towerInnerWidth, T, H, D, iD, iH, finishId)
        break
      case 'rod':
        buildRods(parts, tower, acc, centerX, towerInnerWidth, T, H, D, iD, iH)
        break
      case 'drawer':
        buildDrawers(parts, tower, acc.count, acc.drawerHeight, centerX, towerInnerWidth, T, H, D, iD, finishId)
        break
      case 'shoe_shelf':
        buildShoeShelves(parts, tower, acc.count, centerX, towerInnerWidth, T, H, D, iD, iH, finishId)
        break
